...
const NewBackTop: FC<{id?: string} & BackTopProps> = (props) => <BackTop {...props} />;
...
const MyTable: FC<IProps> = forwardRef((
    ...
    const setFn = useStateDeep({
        // 滚动条 置顶
        setScrollTop: () => {
            const domTop = document.getElementById(state.domTopId);
    ...
    return (
        <div className={`zzzz-table ${className || ''}`} id={state.domId} style={style}>
            ...
            <Exhibit rIf={!!showTopDiv && !!state.showTopButton}>
                <NewBackTop id={state.domTopId} visibilityHeight={state.showTopButton} />
            </Exhibit>
        </div>
    );
});
